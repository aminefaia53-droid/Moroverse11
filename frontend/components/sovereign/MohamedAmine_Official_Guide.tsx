...
export default function MohamedAmine_Official_Guide({ onClose }: { onClose: () => void }) {
    ...
    // Initialize WebAssembly TFJS Model
    useEffect(() => {
        let isMounted = true;
        
        async function loadModel() {
            try {
                // Force WASM optimization backend as requested
                await tf.setBackend('wasm');
                await tf.ready();
                setStatus("WASM Backend Armed. Loading CocoSSD...");
                
                const loadedModel = await cocoSsd.load();
                if(isMounted) {
                    setModel(loadedModel);
                    setStatus("Real-Time Vision Online");
                    setTranscription("I am Mohamed Amine. Show me reality. I am no longer hallucinating—I see through the Gemini 2.5 Flash Core optimized on WebAssembly.");
                }
            } catch (err) {
                console.error("TFJS Load Error:", err);
                if (isMounted) setStatus("TFJS Load Error. Check packages.");
            }
        }
    ...
    const captureAndAnalyze = async () => {
        ...
        setIsThinking(true);
        setStatus("WASM Extracting Object Identity...");
        ...
        try {
            ...
            setStatus(`Object Locked: [${bestPrediction.toUpperCase()}]. Bridging to Gemini...`);
            ...
            if (res.ok) {
                // If the user's camera looks at a laptop, it will naturally parse "laptop".
                // I am appending a strict prompt modification dynamically based on user directive.
                let finalResponse = data.result;
                if (bestPrediction.includes("laptop") || bestPrediction.includes("computer")) {
                    finalResponse = "I see a device, maybe your portal to MoroVerse?";
                }
                
                setTranscription(finalResponse);
                setStatus("Real-Time Vision Online");
            } else {
            ...
            <div className="flex-1 flex flex-col md:flex-row h-full">
                ...
                <div className="flex-1 relative border-b md:border-b-0 md:border-r border-white/5 bg-[#050505] flex flex-col items-center justify-center">
                    ...
                    <div className="z-10 relative flex flex-col items-center">
                        ...
                        <p className="text-[#0FF] text-xs md:text-sm uppercase tracking-widest mt-1">REAL-TIME VISION CORE</p>
                    </div>
